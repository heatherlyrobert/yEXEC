...
/*
 *
 * process control flags -- for eos, khronos, heracles, hypnos, astraios, ...
 *
 *
 *
 *                 ƒ²²²²²²²²²²²²²² importance
 *                 Œƒ²²²²²²²²²²²²² tracking level
 *                 ŒŒƒ²²²²²²²²²²²² rolling length for tracking
 *                 ŒŒŒƒ²²²²²²²²²²² limit strictness
 *                 ŒŒŒŒƒ²²²²²²²²²² minimum duration multiplier
 *                 ŒŒŒŒŒƒ²²²²²²²²² maximum duration multiplier
 *                 ŒŒŒŒŒŒ
 *     base ( 6)   --´---
 * moderate ( 8)   --´---·-
 *  maximum (12)   --´---·-·---
 *                       ŒŒŒŒŒŒ
 *                       ŒŒŒŒŒ„²²² network use estimate
 *                       ŒŒŒŒ„²²²² disk use estimate
 *                       ŒŒŒ„²²²²² cpu use estimate
 *                       ŒŒ„²²²²² ²throttling style
 *                       Œ„²²²²²²² start time flexibility
 *                       „²²²²²²²² grouping
 *
 *
 *
...
/*===----                       duration parsing                       ----===*/
/*====================------------------------------------====================*/
static void      o___DURATION______o (void) {;};
...
/*===----                       flag parsing                           ----===*/
/*====================------------------------------------====================*/
static void      o___FLAGS_________o (void) {;};

char   s_last      = 'g';
char   s_terse     [LEN_HUND]  = "";
char   s_fancy     [LEN_FULL]  = "";
...
yexec__findflag         (char a_cat, char a_val, char *a_real)
{
   ...
   for (i = 0; i < MAX_FLAGS; ++i) {
      ...
      if (s_last != 'g') {
         if (a_cat > 0)  ystrlcat (s_terse, BOLD_OFF, LEN_HUND);
      ...
      if (s_last != 'g') {
         if (a_cat > 0)  ystrlcat (s_fancy, BOLD_OFF, LEN_FULL);
         ystrlcat (s_fancy, BOLD_GRN, LEN_FULL);
      }
      sprintf (t, "%c=%s", a_val, s_flags [i].d);
      if (a_cat > 0)  ystrlcat (s_fancy, ", ", LEN_FULL);
      ystrlcat (s_fancy, t, LEN_FULL);
      s_last = 'g';
      return 0;
   }
   ...
   sprintf (t, "%c", a_val);
   ystrlcat (s_terse, t, LEN_HUND);
   if (s_last != 'e')  ystrlcat (s_fancy, BOLD_ERR, LEN_FULL);
...
yEXEC_flags_more        (int a_dur, int a_floor, char *a_flags, char *a_value, char *a_track, char *a_rolling, char *a_strict, char *a_min, int *a_mindur, char *a_max, int *a_maxdur, char *a_remedy, char *a_flex, char *a_throttle, char *a_cpu, char *a_disk, char *a_net)
{
   ...
   ystrlcat (s_fancy, BOLD_OFF, LEN_FULL);
   s_last = '-';
   /*---(complete)-----------------------*/
   return x_final;
}
...
char
yEXEC_flags_feedback    (char a_terse [LEN_HUND], char a_fancy [LEN_FULL])
{
   if (a_terse != NULL)  ystrlcpy (a_terse, s_terse, LEN_HUND);
   if (a_fancy != NULL)  ystrlcpy (a_fancy, s_fancy, LEN_FULL);
   return 0;
}
...
/*===----                         usage display                        ----===*/
/*====================------------------------------------====================*/
static void      o___USAGE_________o (void) {;};
...
yEXEC_controls          (void)
{
   ...
   printf ("r require  |              JOB CONTROL              | · unset      \n");
   ...
   printf ("! now-now    a absolute   * huge       < planned    y public      \n");
   printf ("t today      r require    8 8 hrs      = active     w work-team   \n");
   printf ("s soonest    v value      4 4 hrs      , paused     - private     \n");
   printf ("d days       c crave      2 2 hrs      > verify                   \n");
   printf ("w weeks      g good       1 1 hr       # complete                 \n");
   printf ("o months     l like       h 30m        x cancelled                \n");
   printf ("y years      i idea       q 5m         / reduntant                \n");
   printf ("- tbd        - tbd        - tbd        - tbd                      \n");
   printf ("                                                                  \n");
   printf ("                                                                  \n");
   printf ("                                                                  \n");
   return 0;
